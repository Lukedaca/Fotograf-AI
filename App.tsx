...
function App() {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      const syncFromLocation = () => {      if (typeof window === 'undefined') return;
  ...
  if (view === 'gallery-preview' && galleryProjectId) {
    return (
      <Suspense fallback={<div className="flex-1 flex items-center justify-center text-white/50 animate-pulse">Načítání galerie...</div>}>
  ...
  if (view === 'home') {
    return (
      <Suspense fallback={<div className="flex-1 flex items-center justify-center text-white/50 animate-pulse">Start systému...</div>}>
  ...
  return (
    <div className="h-screen w-screen overflow-hidden flex font-sans text-white relative">
        ...
        {/* Main Content Area */}
        <main className={`flex-1 flex flex-col h-full relative z-10 p-4 pl-0 transition-all duration-500`}>
            {/* Glass Container for Main View */}
            <div className="w-full h-full os-glass rounded-[2rem] overflow-hidden flex flex-col border border-white/10 relative">
                <Suspense fallback={<div className="flex-1 flex items-center justify-center text-white/30">Načítání...</div>}>
                  {renderView()}
                </Suspense>
            </div>
        </main>
        ...
        <div className="fixed bottom-6 right-6 z-[250] w-auto flex flex-col space-y-2">
            {notifications.map(n => (
                <div key={n.id} className={`flex items-center px-4 py-3 rounded-2xl shadow-xl backdrop-blur-xl border text-sm animate-fade-in-right ${n.type === 'error' ? 'bg-red-500/20 border-red-500/30 text-white' : 'bg-white/10 border-white/20 text-white'}`}>
                    <span className="mr-4">{n.message}</span>
                    <button onClick={() => setNotifications(current => current.filter(notif => notif.id !== n.id))} className="text-white/50 hover:text-white">
                        ✕
                    </button>
                </div>
            ))}
